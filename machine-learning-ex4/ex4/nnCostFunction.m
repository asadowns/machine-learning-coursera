...
a2 = sigmoid(XInt * Theta1');
a2Int = [ones(m, 1) a2];

%a3 = h
h = sigmoid(a2Int * Theta2');
...
theta2Temp = Theta2;
theta2Temp(:,1) = 0;
theta1Reg = sum(sum(theta1Temp .^2));
theta2Reg = sum(sum(theta2Temp .^2));

regularizationParameter = ((lambda/(2 * m)) * (theta1Reg + theta2Reg));

J = ((1/m) * sum(sum(((-yVec .* log(h')) - ((1 - yVec) .* log(1 - h')))))) + regularizationParameter;


for t=1:m,
    %Step 1 Feed forward for t-th example
    Theta1;
	a1 = XInt(t, :)';
	z2 = Theta1 * a1;
...
%Failed vectorized approach
% δ(2) =  Θ(2) T δ(3). ∗ g′(z(2))
%a2Int
%h
%gZ2 = a2 .* (1 - a2)
%firstTerm = d3 * Theta2'
%d2 = (theta2Temp' * d3) .* sigmoidGradient(a2Int)
